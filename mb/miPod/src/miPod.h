...
typedef struct {
	char master_pub[PUB_KEY_SIZE];
	char user_pub[PUB_KEY_SIZE]; // user public key should be mapped
	/*
	 * map goes here
	 * the map should hold the key value pair needed to
	 * unlock the song for a user.
	 * a comparison should be done between the private key generated
	 * and the key stored in the map. if successful then the master key
	 * can be unlocked
	 *
	 */
	map mp;
...
typedef volatile struct __attribute__((__packed__)) {
    ...
    char pin[MAX_PIN_SZ];       // stores logged in or attempted pin (private key is derivate of user pin)
